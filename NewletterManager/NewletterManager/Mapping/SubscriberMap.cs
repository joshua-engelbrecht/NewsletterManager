...
namespace NewsletterManager.Mapping
{
    ...
    {
        public int addSubscriber(Subscribers subscriber, SqlConnection conn)
        {
        ...
        public Subscribers getSubscriber(string id, SqlConnection conn)
        {
        ...
        public List<Subscribers> getSubscribers(Subscribers searchCriteria, SqlConnection conn)
        {
        ...
        public int deleteSubscriber(string id, SqlConnection conn)
        {
            var rtn = 0;
        ...
        public int editSubscriber(Subscribers subscriber, SqlConnection conn)
        {
            var CRHE = subscriber.CanReceiveHtmlEmail ? 1 : 0;
            var CRA = subscriber.CanReceiveAttachment ? 1 : 0;
            var rtn = 0;
        ...
        internal void editSubscribers(List<Subscribers> editedSubscribers, SqlConnection conn)
        {
            foreach (Subscribers subscriber in editedSubscribers)
            {
                editSubscriber(subscriber, conn);
            }
        }
    }
}
